...
#endif
    gtk_init(&argc, &argv);
    /* Load our territory CSV once at startup.
       Assumes Countries_tiles.csv sits next to the binary. */
    tool_init("src/Countries_tiles.csv");
...
static void gpredict_app_create()
{
    ...
    title = g_strdup(_("OGpredict"));
    icon = logo_file_name("gpredict_icon_color.png");
