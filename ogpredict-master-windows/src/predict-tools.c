...
/* Provide jd_to_gregorian without depending on invjday().
 * Algorithm: Fliegel–Van Flandern for the calendar date + fractional-day for time.
 */
#ifndef GPREDICT_HAVE_JD_TO_GREGORIAN
static inline void ogrp_jd_to_greg(double jd,
                                   int *Y, int *Mo, int *D,
                                   int *h, int *m, int *s)
{
    /* Convert astronomical JD to civil date/time.
       We use JD + 0.5 to move the epoch from noon to midnight. */
    const double zf = floor(jd + 0.5);
    const long Z = (long)zf;
    double F = (jd + 0.5) - zf;              /* fractional day in [0,1) */
    ...
    long day = B - Dd - (long) (30.6001 * E);
    int month = (E < 14) ? (int)(E - 1) : (int)(E - 13);
    int year  = (month > 2) ? (int)(C - 4716) : (int)(C - 4715);

    /* Time of day from fractional part (round to nearest second) */
    double sec_total = F * 86400.0;
    int ih = (int) (sec_total / 3600.0);
    sec_total -= ih * 3600.0;
    int im = (int) (sec_total / 60.0);
    int is = (int) floor(sec_total - im * 60.0 + 0.5);
    /* normalize rounding overflow */
    if (is >= 60) { is -= 60; im += 1; }
    if (im >= 60) { im -= 60; ih += 1; }
    if (ih >= 24) { ih -= 24; day += 1; /* rare carry; full normalization not needed here */ }

    *Y = year; *Mo = month; *D = (int)day;
    *h = ih; *m = im; *s = is;
}
#define jd_to_gregorian ogrp_jd_to_greg
#endif

gchar *
predict_timestamp_to_string(sat_t *sat, int t)
{
    /* sat->jul_utc is in days; convert to seconds and add t, then back to days */
    double jd = sat->jul_utc + ((double)t)/86400.0;
    /* now split into Y/M/D H:M:S */
    int Y, Mo, D, h, m, s;
    jd_to_gregorian(jd, &Y, &Mo, &D, &h, &m, &s);
    return g_strdup_printf("%04d/%02d/%02d %02d:%02d:%02d",
                           Y, Mo, D, h, m, s);
}

/* pull the SGP4‐computed “sub‐satellite” lat/lon right out of the sat struct */
void
predict_get_subsatellite_coords(sat_t *sat, double *lat, double *lon)
{
    *lat = sat->ssplat;
    *lon = sat->ssplon;
}
